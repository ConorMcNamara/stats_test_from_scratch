...
def levene_test(*args) -> Tuple[float, float]:
    ...
    w = (
        scalar
        * np.sum(n_i * np.power(z_bar_condensed - np.mean(z_bar), 2))
        / np.sum(np.power(all_z_ij - z_bar, 2))
    )
...
def one_way_f_test(*args) -> Tuple[float, float]:
    ...
    explained_variance = np.sum(
        n_i * np.power(y_bar_condensed - np.mean(all_y_ij), 2) / (k - 1)
    )
...
def cochran_q_test(*args) -> Tuple[float, float]:
    ...
    T = (
        scalar
        * np.sum(np.power(col_sum - (N / k), 2))
        / np.sum(row_sum * (k - row_sum))
    )
    p = 1 - chi2.cdf(T, df)
    return T, p
...
def jonckheere_trend_test(*args, **kwargs) -> Tuple[float, float]:
    ...
    if len(np.unique(u)) != 1:
        raise AttributeError(
            "Jonckheere Test requires that each group have the same number of observations"
        )
    ...
    part_one = (
        2 * (pow(n, 3) - sum_t_3 - sum_u_3)
        + 3 * (pow(n, 2) - sum_t_2 - sum_u_2)
        + 5 * n
    ) / 18
    part_two = (
        (sum_t_3 - 3 * sum_t_2 + 2 * n)
        * (sum_u_3 - 3 * sum_u_2 + 2 * n)
        / (9 * n * (n - 1) * (n - 2))
    )
