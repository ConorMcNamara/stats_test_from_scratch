...
def tukey_fence_test(data, coef=1.5):
...
def tietjen_moore_test(data, num_outliers=1, alternative='two-sided', alpha=0.05):
    ...
    def teitjen(data, num_outliers=1, alternative='two-sided', simulation=True):
        ...
        if alternative.casefold() == 'greater':
            ...
            l = np.sum(np.power(data_large - y_bar_large, 2)) / np.sum(np.power(data - y_bar, 2))
            ...
            l = np.sum(np.power(data_small - y_bar_small, 2)) / np.sum(np.power(data - y_bar, 2))
        else:
...
def dixon_q_test(data, alpha=0.01):
    """Not found in either scipy.stats or statsmodels
